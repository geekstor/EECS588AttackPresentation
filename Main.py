...
def get_batch(size, partial_answer=False, step=False):
    ...
    for i in range(size):
        ...
        if not partial_answer:
            ...
            rand_end_char = random.randint(0, len(list(labeled_samples[rand_key][rand_ind][1] - 1)))
            partial_answer_list.append(labeled_samples[rand_key][rand_ind][1][:rand_end_char])
            rest_answer_list.append(labeled_samples[rand_key][rand_ind][1][rand_end_char:])
    ...
    if not partial_answer:
        ...
        return [numpy.array(img_list, dtype=float), partial_answer_list, rest_answer_list]
