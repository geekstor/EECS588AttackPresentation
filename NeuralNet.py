...
print(ae_model.layers)

ae_model.layers.remove(0)

for i in range(6):
    ae_model.layers.remove(len(ae_model.layers))
...
ae_model.add(RepeatVector(max_caption_len))
...
ae_model.fit([Main.get_batch(5, True)[0], Main.get_batch(5, True)[1]], Main.get_batch(5, True, True)[2], nb_epoch=20000, verbose=1, batch_size=5,
